...
<head>
	...
	<title>ITWORLD - [ 내 단어장 리눅스 마스터 ]</title>
...
<body>
		<input type="hidden" id="category" value="word_linux">
			...
			<div id="dictionary">
				<span class="animate-box"><a href="MynoteIPE.no" class="dictionary_name">정보처리기사</a></span> 
	...
	<div class="all">
		...
		<%if(nowpage<=1){ //1페이지 이하인 경우 %>
			...
			<%}else{  //2페이지 이상인 경우 - 한 페이지 적은 페이지로 이동 %>
				<a class="mypagelist" href="./MynoteLinux.no?page=<%=nowpage-1 %>">
				[이전]</a>&nbsp;
			<%} %>
			...
			<%	}else{ //현재 페이지 아닌 경우 링크 겁니다.	%>
				<a class="mypagelist" href="./MynoteLinux.no?page=<%=a %>"><%=a %></a>
				<%} %>
			<%}%>
		...
		<%if(nowpage >= maxpage){ %>
			[다음]
		<%}else{ %>
			<a class="mypagelist" href="./MynoteLinux.no?page=<%=nowpage+1 %>">[다음]</a>
		<%} %>	
		</div>
		</div> 

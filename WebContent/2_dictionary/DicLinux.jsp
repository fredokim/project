...
<head>
	...
	<title>ITWORLD - [ 용어사전 - 리눅스마스터 ]</title>
...
<body>
	<input type="hidden" id="category" value="word_linux">
	...
	<div id="dictionary">
		<span class="animate-box"><a href="DicIPE.di" class="dictionary_name">정보처리기사</a></span> 
		<span class="animate-box"><a href="DicSQL.di" class="dictionary_name"> SQL-D</a></span> 
		<span class="animate-box"><a class="dictionary_name now_page"> 리눅스 마스터</a></span> 
		<span class="animate-box"><a href="DicETC.di" class="dictionary_name"> 기타실무용어</a> &nbsp;</span>
	</div>
	...
	<%	for(int i=0;i<dicdatalist.size();i++){
		...
		<%if(nowpage<=1){ //1페이지 이하인 경우 %>
			...
			<%}else{  //2페이지 이상인 경우 - 한 페이지 적은 페이지로 이동 %>
				<a class="pagelist" href="./DicLinux.di?page=<%=nowpage-1 %>">
				[이전]</a>&nbsp;
			<%} %>
			...
			<%	}else{ //현재 페이지 아닌 경우 링크 겁니다.	%>
				<a class="pagelist" href="./DicLinux.di?page=<%=a %>"><%=a %></a>
				<%} %>
			<%} %>
		...
		<%if(nowpage >= maxpage){ %>
			[다음]
		<%}else{ %>
			<a class="pagelist" href="./DicLinux.di?page=<%=nowpage+1 %>">[다음]</a>
		<%} %>	
		</div>
		</div> 
